...
describe Algorithmable::Graphs::Traversals::BreadthFirst do
  context '#traversal' do
    ...
    it do
      expect(traversal.path_to(3)).to eq([0, 5, 3])
    end
    ...
    it do
      expect(traversal.path_to(6)).to eq([0, 6])
    end
  end

  context 'additional BFS traversal test' do
    let(:graph) do
    ...
    it 'can find path shortest path from 0 to ' do
      expect(traversal.path_to(3)).to eq([0, 2, 3])
    end
  end
end
